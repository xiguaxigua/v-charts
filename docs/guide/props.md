...
图表自身的属性，例如用于设置数据类型的 `dataType`, 是否展示为面积图的 `area`，这样的属性被置于
...
| 配置项 | 简介 | 类型 | 默认值 |
| ----- | --- |:----:| --- |
| data | 数据，[参考文档](/guide/data.html) | object | - |
| settings | 配置项 | object | - |
| width | 宽度 | string | auto |
| height | 高度 | string | 400px |
| events | 事件绑定，[参考文档](/guide/event.html) | object | - |
...
与echarts配置项对应的属性也被加到了组件上，用于直接覆盖options原有的对应属性，使用方式可参考[文档](http://echarts.baidu.com/option.html)
